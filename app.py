...
st.caption(f"📺 TradingView confirmation: **{tv_coverage}/{len(df)} symbols** • 30m / 1h / 4h / 1D / 1W / 1M technical layer • Binance remains the primary live feed")
...
st.dataframe(entry_view[entry_cols], use_container_width=True, hide_index=True, column_config={
    ...
    "tv_1d_rsi": st.column_config.NumberColumn("TV RSI 1D", format="%.1f"),
                "tv_1w_rsi": st.column_config.NumberColumn("TV RSI 1W", format="%.1f"),
                "tv_1m_rsi": st.column_config.NumberColumn("TV RSI 1M", format="%.1f"),
    "tv_1w_rsi": st.column_config.NumberColumn("TV RSI 1W", format="%.1f"),
    "tv_1m_rsi": st.column_config.NumberColumn("TV RSI 1M", format="%.1f"),
