#!/usr/bin/env python3
"""Pump Replay / Backtest Engine v5 - persistent, BTC-relative and historically calibrated."""
...
def evaluate(df,btc,symbol):
    ...
    # Historical calibration is strictly walk-forward: only earlier episodes contribute.
    hist=[];episodes=[];calibration=[]
    for z in raw:
        prior=[q for q in episodes if pd.Timestamp(q["time"])<pd.Timestamp(z["time"])]
        samples=len(prior);hits=sum((q["future_max_gain"]["240"] or 0)>=10 for q in prior)
        rate=hits/samples*100 if samples else 0
        hist_quality=min(rate/25,1)*100 if samples>=5 else 50
        persistence_quality=min(z["persistence"]/3,1)*100
        v5=round(.70*z["v4_score"]+.15*persistence_quality+.15*hist_quality)
        if samples>=5 and rate>=8 and z["persistence"]>=3 and v5>=72 and z["v4_alert_quality"]=="A":grade="A"
        elif samples>=5 and rate>=8 and z["persistence"]>=2 and v5>=65 and z["v4_alert_quality"] in ("A","B"):grade="B"
        elif v5>=55 and z["persistence"]>=1:grade="WATCH"
        else:grade="REJECT"
        z.update({"v5_score":max(0,min(v5,100)),"v5_persistence":z["persistence"],"v5_hist_samples":samples,"v5_hist_hit_rate_240m":round(rate,2),"v5_grade":grade,"v5_alert":grade in ("A","B") and v5>=65 and z["persistence"]>=2 and samples>=5 and rate>=8})
    ...
    for lo,hi in ((50,59),(60,69),(70,79),(80,89),(90,100)):
        q=[z for z in episodes if lo<=z["v5_score"]<=hi];h60=[z for z in q if (z["future_max_gain"]["60"] or 0)>=10];h240=[z for z in q if (z["future_max_gain"]["240"] or 0)>=10];h20=[z for z in q if (z["future_max_gain"]["240"] or 0)>=20]
    ...
    return {"symbol":symbol,"status":"ok","bars":len(x),"start":x.time.iloc[0].isoformat(),"end":x.time.iloc[-1].isoformat(),"detections":len(episodes),"v5_alerts":len(a),"hit_60m_ge10":sum((z["future_max_gain"]["60"] or 0)>=10 for z in a),"hit_240m_ge10":sum((z["future_max_gain"]["240"] or 0)>=10 for z in a),"hit_240m_ge20":sum((z["future_max_gain"]["240"] or 0)>=20 for z in a),"v5_alert_precision_240m_ge10_pct":round(sum((z["future_max_gain"]["240"] or 0)>=10 for z in a)/len(a)*100,2) if a else 0,"v5_alert_precision_240m_ge20_pct":round(sum((z["future_max_gain"]["240"] or 0)>=20 for z in a)/len(a)*100,2) if a else 0,"score_band_precision":bands,"v5_alert_episodes":a[:150],"episodes":episodes[:150]}

def main():
    ap=argparse.ArgumentParser();ap.add_argument("--symbols",default="NOMUSDT,NILUSDT,STXUSDT,RAYUSDT,SEIUSDT,SUIUSDT,INJUSDT,AVAXUSDT,PUMPUSDT");ap.add_argument("--start",default="2026-09-20T00:00:00Z");ap.add_argument("--end",default="2026-09-25T00:00:00Z");ap.add_argument("--output",default="data/backtest_results_v5.json");a=ap.parse_args()
    btc=fetch("BTCUSDT",a.start,a.end);out=[];cal={}
    for s in [z.strip().upper() for z in a.symbols.split(",") if z.strip() and z.strip().upper()!="BTCUSDT"]:
        print("Replaying",s)
        try:
            r=evaluate(fetch(s,a.start,a.end),btc,s);out.append(r)
            if r.get("status")=="ok":cal[s]={"samples":r["detections"],"hit_rate_240m_ge10_pct":r["v5_alert_precision_240m_ge10_pct"]}
        except Exception as e:out.append({"symbol":s,"status":"error","error":str(e)})
    payload={"generated_at":datetime.now(timezone.utc).isoformat(),"engine":"Pump Replay / Backtest v5","data_source":BASE,"method":"Binance 1m Spot klines with BTC-relative strength, persistent confirmation and walk-forward historical calibration","v5_design":{"v4_weight":0.70,"persistence_weight":0.15,"historical_weight":0.15,"action_min_score":65,"min_persistence":2,"min_historical_samples":5,"min_historical_hit_rate_pct":8,"targets":["10% in 60m","10% in 240m","20% in 240m"]},"calibration":cal,"results":out}
