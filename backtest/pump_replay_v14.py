...
def classify_v14(r, persistence=0, confirmation=False, efficiency=0.0):
    """Contemporaneous multi-factor pump-stage classifier; no future bars."""
    k=float(r.get("stochrsi_k",50) or 50); slope=float(r.get("stochrsi_slope",0) or 0)
    cross_up=bool(r.get("stochrsi_cross_up",False))
    vr=float(r.get("volume_ratio",0) or 0); ta=float(r.get("trade_accel",0) or 0)
    bp=float(r.get("buy_pressure",.5) or .5); p1=float(r.get("ret_1m",0) or 0)
    p5=float(r.get("ret_5m",0) or 0); rs5=float(r.get("relative_strength_5m",0) or 0)
    ...
    activity=clamp(max(vr-1,0)/2)*100
    accel=clamp(max(ta-1,0)/2)*100
    flow=clamp((bp-.50)/.20)*100
    momentum=clamp((p5+.25)/2.5)*100
    rel=clamp((rs5+.25)/1.25)*100
...
def evaluate(df,btc,symbol,calibration=None):
    ...
    for z in raw:
        ...
        z["v12_efficiency"]=round(efficiency,4)
        z["v12_adverse_extension"]=round(adverse_extension,4)
        z["v12_second_candle_confirm"]=bool(second_confirm)
        z["v12_second_candle_hold"]=bool(second_hold)
        z["v12_confirmation"]=bool(confirmation)
        ...
        # V14: classify the live pump stage after all contemporaneous features
        # are available. No future gain/MAE/MFE fields are used by this classifier.
        v14=classify_v14(z,persistence=persistence,confirmation=confirmation,efficiency=efficiency)
        z.update(v14)
        ...
        z.update({
            "v12_score":score,"v12_persistence":persistence,"v14_stage":v14["v14_stage"],"v14_stoch_state":v14["v14_stoch_state"],"v14_stage_score":v14["v14_stage_score"],"v14_stage_evidence":v14["v14_stage_evidence"],"v14_stage_confidence":v14["v14_stage_confidence"],"pump_stage":("EARLY" if stoch_early else "EXPANSION" if stoch_expansion else "EXHAUSTION" if stoch_exhaustion else "COOLDOWN" if z["stochrsi_stage"]=="COOLDOWN" else "WATCH"),
    ...
    a=[z for z in episodes if z["v12_alert"]]
    bands={}
...
def expansion_matrix(items):
    ...
    for label,group in groups.items():
        ...
        for h in horizons:
            ...
            vals=[]
            for z in group:
                v=z.get("future_max_gain",{}).get(gkey)
                if v is not None: vals.append(v)
            for target in targets:
                hits=sum(v>=target for v in vals)
                d[f"hit_{target}pct_{h}m"]=hits
                d[f"precision_{target}pct_{h}m"]=round(hits/len(vals)*100,2) if vals else 0
            d[f"avg_max_gain_{h}m_pct"]=round(sum(vals)/len(vals),2) if vals else 0
            d[f"median_max_gain_{h}m_pct"]=round(float(pd.Series(vals).median()),2) if vals else 0
        out[label]=d
    return out
