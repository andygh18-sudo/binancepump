...
V14 is a copy of the repository's V13 engine with the multi-factor V14
stage classifier retained and the output filenames made V14-specific.
"""
...
def evaluate(df,btc,symbol,calibration=None):
    ...
    for z in raw:
        ...
        if avoid:path="AVOID";alert=False;grade="REJECT"
        elif confirmed_path and score>=70:path="CONFIRMED";alert=True;grade="A"
        elif early_path and score>=62:path="EARLY";alert=True;grade="B"
        elif score>=52 and persistence>=1:path="WATCH";alert=False;grade="WATCH"
        else:path="REJECT";alert=False;grade="REJECT"
        z.update({"v12_score":score,"v12_persistence":persistence,"pump_stage":("EARLY" if stoch_early else "EXPANSION" if stoch_expansion else "EXHAUSTION" if stoch_exhaustion else "COOLDOWN" if z["stochrsi_stage"]=="COOLDOWN" else "WATCH"),"v12_reliability":round(reliability,2),"v12_reliability_modifier":round(reliability_modifier,2),"v12_grade":grade,"v12_path":path,"v12_alert":bool(alert),"v12_confirmation":bool(confirmation),"v12_a_plus":bool(a_plus),"v12_follow_strength":follow_strength,"v12_follow_ok":bool(v12_follow_ok),"v12_efficiency":round(efficiency,4),"v12_adverse_extension":round(adverse_extension,4),"v8_hist_samples":samples,"v8_hist_hit_rate_240m":round(rate,2),"v8_smoothed_hist_rate_240m":round(smooth_rate,2),"v8_hist_modifier":round(hist_modifier,2),"v8_grade":grade,"v8_path":path,"v8_early_path":bool(early_path),"v8_confirmed_path":bool(confirmed_path),"v8_avoid":bool(avoid),"v8_btc_risk_off":bool(btc_risk_off),"v8_alert":bool(alert)})
...
def main():
    ap=argparse.ArgumentParser();ap.add_argument("--symbols",default="NOMUSDT,NILUSDT,STXUSDT,RAYUSDT,SEIUSDT,SUIUSDT,INJUSDT,AVAXUSDT,PUMPUSDT");ap.add_argument("--start",default="2026-09-20T00:00:00Z");ap.add_argument("--end",default="2026-09-25T00:00:00Z");ap.add_argument("--train-start",default="2026-09-13T00:00:00Z");ap.add_argument("--output",default="data/backtest_results_v14.json");a=ap.parse_args()
    ...
    os.makedirs(os.path.dirname(a.output) or ".",exist_ok=True);json.dump(payload,open(a.output,"w"),indent=2);json.dump(cal,open("data/v14_calibration.json","w"),indent=2);print(json.dumps(payload,indent=2))
