#!/usr/bin/env python3
"""Pump Replay / Backtest Engine v7 - precision-first confluence with persistent early-entry and reliability gating."""
...
def evaluate(df,btc,symbol,calibration=None):
    ...
    for z in raw:
        # V7 reliability is neutral until the calibration has enough samples.
        hist_quality=min(rate/25,1)*100 if samples>=20 else 50
        persistence_quality=min(z["persistence"]/3,1)*100
        v7=round(.75*z["v4_score"]+.10*persistence_quality+.15*hist_quality)

        # Precision-first early entry: no one-observation exception.
        early_exception=(
            z["v4_alert_quality"] in ("A","B") and
            z["v4_score"]>=72 and
            z["v4_confirmations"]>=7 and
            z["persistence"]>=2 and
            z["buy_pressure"]>=.60 and
            z["volume_ratio"]>=2.0 and
            z["trade_accel"]>=2.0 and
            z["relative_strength_5m"]>0.50 and
            z["ret_5m"]>0 and
            z["ret_1m"]<2.5 and
            z["distance_to_resistance_pct"]<=1.5
        )

        if samples>=20 and rate>=8 and z["persistence"]>=3 and v7>=72 and z["v4_alert_quality"]=="A":
            grade="A"
        elif samples>=20 and rate>=8 and z["persistence"]>=2 and v7>=65 and z["v4_alert_quality"] in ("A","B"):
            grade="B"
        elif early_exception and v7>=68:
            grade="EARLY"
        elif v7>=58 and z["persistence"]>=1:
            grade="WATCH"
        else:
            grade="REJECT"

        z.update({
            "v7_score":max(0,min(v7,100)),
            "v7_persistence":z["persistence"],
            "v7_hist_samples":samples,
            "v7_hist_hit_rate_240m":round(rate,2),
            "v7_grade":grade,
            "v7_early_exception":early_exception,
            "v7_alert":(
                (grade=="EARLY" and v7>=68 and z["persistence"]>=2) or
                (grade in ("A","B") and v7>=65 and z["persistence"]>=2 and samples>=20 and rate>=8)
            )
        })
        if not episodes or (pd.Timestamp(z["time"])-pd.Timestamp(episodes[-1]["time"])).total_seconds()>=1800:
            episodes.append(z)
        elif z["v7_score"]>episodes[-1]["v7_score"]:episodes[-1]=z
    a=[z for z in episodes if z["v7_alert"]]
    bands={}
    for lo,hi in ((50,59),(60,69),(70,79),(80,89),(90,100)):
        q=[z for z in episodes if lo<=z["v7_score"]<=hi];h60=[z for z in q if (z["future_max_gain"]["60m"] or 0)>=10];h240=[z for z in q if (z["future_max_gain"]["240m"] or 0)>=10];h20=[z for z in q if (z["future_max_gain"]["240m"] or 0)>=20]
    ...
    return {"symbol":symbol,"status":"ok","bars":len(x),"start":x.time.iloc[0].isoformat(),"end":x.time.iloc[-1].isoformat(),"detections":len(episodes),"v7_alerts":len(a),"hit_60m_ge10":sum((z["future_max_gain"]["60m"] or 0)>=10 for z in a),"hit_240m_ge10":sum((z["future_max_gain"]["240m"] or 0)>=10 for z in a),"hit_240m_ge20":sum((z["future_max_gain"]["240m"] or 0)>=20 for z in a),"v7_alert_precision_240m_ge10_pct":round(sum((z["future_max_gain"]["240m"] or 0)>=10 for z in a)/len(a)*100,2) if a else 0,"v7_alert_precision_240m_ge20_pct":round(sum((z["future_max_gain"]["240m"] or 0)>=20 for z in a)/len(a)*100,2) if a else 0,"score_band_precision":bands,"v7_alert_episodes":a[:150],"episodes":episodes[:150]}

def main():
    ap=argparse.ArgumentParser();ap.add_argument("--symbols",default="NOMUSDT,NILUSDT,STXUSDT,RAYUSDT,SEIUSDT,SUIUSDT,INJUSDT,AVAXUSDT,PUMPUSDT");ap.add_argument("--start",default="2026-09-20T00:00:00Z");ap.add_argument("--end",default="2026-09-25T00:00:00Z");ap.add_argument("--train-start",default="2026-09-13T00:00:00Z");ap.add_argument("--output",default="data/backtest_results_v7.json");a=ap.parse_args()
    ...
    payload={"generated_at":datetime.now(timezone.utc).isoformat(),"engine":"Pump Replay / Backtest v7","data_source":BASE,"method":"Binance 1m Spot klines with BTC-relative strength, independent pre-test calibration, persistent confirmation and precision-first early-entry gating","v7_design":{"v4_weight":0.75,"persistence_weight":0.10,"historical_weight":0.15,"action_min_score":65,"min_persistence":2,"min_historical_samples":20,"min_historical_hit_rate_pct":8,"early_entry_exception":True,"early_v4_min_score":72,"early_min_confirmations":7,"early_min_relative_strength_5m_pct":0.50,"targets":["10% in 60m","10% in 240m","20% in 240m"]},"calibration":cal,"results":out}
    os.makedirs(os.path.dirname(a.output) or ".",exist_ok=True);json.dump(payload,open(a.output,"w"),indent=2);json.dump(cal,open("data/v7_calibration.json","w"),indent=2);print(json.dumps(payload,indent=2))
