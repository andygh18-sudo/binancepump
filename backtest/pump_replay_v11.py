#!/usr/bin/env python3
"""Pump Replay / Backtest Engine v11 - second-candle confirmation, efficiency, adverse-extension control and soft reliability."""
...
def evaluate(df,btc,symbol,calibration=None):
    # V11 keeps V9's signal engine but adds confirmation and execution-quality filters.
    ...
    for z in raw:
        ...
        z["v11_efficiency"]=round(efficiency,4)
        z["v11_adverse_extension"]=round(adverse_extension,4)
        z["v11_second_candle_confirm"]=bool(second_confirm)
        z["v11_second_candle_hold"]=bool(second_hold)
        z["v11_confirmation"]=bool(confirmation)
        ...
        # V11 reliability is a soft modifier, never a hard gate.
        ...
        # V11: require sustained follow-through across subsequent observations.
        ...
        v11_follow_ok=follow_strength>=20
        ...
        early_path=(
            ...
            efficiency_ok and extension_ok and confirmation and v11_follow_ok
        )
        confirmed_path=(
            ...
            efficiency>=0.50 and extension_ok and v11_follow_ok
        )
        # Avoid/chase filter.
        ...
        z.update({
            "v11_score":score,"v11_persistence":persistence,
            "v11_reliability":round(reliability,2),
            "v11_reliability_modifier":round(reliability_modifier,2),
            "v11_grade":grade,"v11_path":path,"v11_alert":bool(alert),
            "v11_confirmation":bool(confirmation),
            "v11_follow_strength":follow_strength,"v11_follow_ok":bool(v11_follow_ok),
            "v11_efficiency":round(efficiency,4),
            "v11_adverse_extension":round(adverse_extension,4),
            "v11_score":score,"v11_persistence":persistence,
        ...
        if not episodes or (pd.Timestamp(z["time"])-pd.Timestamp(episodes[-1]["time"])).total_seconds()>=1800:
            episodes.append(z)
        elif z["v11_score"]>episodes[-1]["v11_score"]:episodes[-1]=z
    a=[z for z in episodes if z["v11_alert"]]
    bands={}
    for lo,hi in ((50,59),(60,69),(70,79),(80,89),(90,100)):
        q=[z for z in episodes if lo<=z["v11_score"]<=hi];h60=[z for z in q if (z["future_max_gain"]["60m"] or 0)>=10];h240=[z for z in q if (z["future_max_gain"]["240m"] or 0)>=10];h20=[z for z in q if (z["future_max_gain"]["240m"] or 0)>=20]
    ...
    early_alerts=[z for z in a if z.get("v11_path")=="EARLY"]; confirmed_alerts=[z for z in a if z.get("v11_path")=="CONFIRMED"]
    false_pos=sum((z["future_max_gain"]["240m"] or 0)<10 for z in a)
    return {"symbol":symbol,"status":"ok","bars":len(x),"start":x.time.iloc[0].isoformat(),"end":x.time.iloc[-1].isoformat(),"detections":len(episodes),"v11_alerts":len(a),"v11_early_alerts":len(early_alerts),"v11_confirmed_alerts":len(confirmed_alerts),"v11_false_positives":false_pos,"v11_false_positive_rate_pct":round(false_pos/len(a)*100,2) if a else 0,"hit_60m_ge10":sum((z["future_max_gain"]["60m"] or 0)>=10 for z in a),"hit_240m_ge10":sum((z["future_max_gain"]["240m"] or 0)>=10 for z in a),"hit_240m_ge20":sum((z["future_max_gain"]["240m"] or 0)>=20 for z in a),"v11_alert_precision_240m_ge10_pct":round(sum((z["future_max_gain"]["240m"] or 0)>=10 for z in a)/len(a)*100,2) if a else 0,"v11_alert_precision_240m_ge20_pct":round(sum((z["future_max_gain"]["240m"] or 0)>=20 for z in a)/len(a)*100,2) if a else 0,"score_band_precision":bands,"v11_alert_episodes":a[:150],"episodes":episodes[:150]}

def main():
    ap=argparse.ArgumentParser();ap.add_argument("--symbols",default="NOMUSDT,NILUSDT,STXUSDT,RAYUSDT,SEIUSDT,SUIUSDT,INJUSDT,AVAXUSDT,PUMPUSDT");ap.add_argument("--start",default="2026-09-20T00:00:00Z");ap.add_argument("--end",default="2026-09-25T00:00:00Z");ap.add_argument("--train-start",default="2026-09-13T00:00:00Z");ap.add_argument("--output",default="data/backtest_results_v11.json");a=ap.parse_args()
    ...
    payload={"generated_at":datetime.now(timezone.utc).isoformat(),"engine":"Pump Replay / Backtest v11","data_source":BASE,"method":"Binance 1m Spot klines with V10 features plus efficiency-weighted scoring, multi-candle follow-through and structural confirmation, volume-to-price efficiency, adverse-extension veto and soft walk-forward reliability modifier","v11_design":{"v10_base":True,"v4_weight":0.55,"persistence_weight":0.10,"follow_through_weight":0.10,"efficiency_weight":0.15,"relative_strength_weight":0.05,"second_candle_confirmation":True,"efficiency_filter":True,"adverse_extension_veto":True,"multi_candle_follow_through":True,"historical_hard_gate":False,"early_min_score":65,"confirmed_min_score":72,"targets":["10% in 60m","10% in 240m","20% in 240m"]},"v9_design":{"v4_weight":0.72,"persistence_weight":0.10,"follow_through_weight":0.13,"historical_risk_modifier":True,"historical_hard_gate":False,"smoothed_prior_hit_rate_pct":10,"early_v4_min_score":60,"early_min_follow_through":65,"early_min_persistence":2,"confirmed_v4_min_score":72,"confirmed_min_persistence":3,"confirmed_min_follow_through":75,"avoid_filter":True,"targets":["10% in 60m","10% in 240m","20% in 240m"]},"calibration":cal,"results":out}
    os.makedirs(os.path.dirname(a.output) or ".",exist_ok=True);json.dump(payload,open(a.output,"w"),indent=2);json.dump(cal,open("data/v11_calibration.json","w"),indent=2);print(json.dumps(payload,indent=2))
