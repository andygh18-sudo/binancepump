#!/usr/bin/env python3
"""Pump Replay / Backtest Engine v9 - follow-through and persistence aware dual-path pump detection."""
...
def evaluate(df,btc,symbol,calibration=None):
    ...
    for z in raw:
        ...
        early_path=(
            ...
            not btc_risk_off and follow_through>=65 and not stall
        )
        confirmed_path=(
            ...
            follow_through>=75 and momentum_ok
        )
        # Avoid/chase filter.
    ...
    return {"symbol":symbol,"status":"ok","bars":len(x),"start":x.time.iloc[0].isoformat(),"end":x.time.iloc[-1].isoformat(),"detections":len(episodes),"v9_alerts":len(a),"v9_early_alerts":len(early_alerts),"v9_confirmed_alerts":len(confirmed_alerts),"v9_false_positives":false_pos,"v9_false_positive_rate_pct":round(false_pos/len(a)*100,2) if a else 0,"hit_60m_ge10":sum((z["future_max_gain"]["60m"] or 0)>=10 for z in a),"hit_240m_ge10":sum((z["future_max_gain"]["240m"] or 0)>=10 for z in a),"hit_240m_ge20":sum((z["future_max_gain"]["240m"] or 0)>=20 for z in a),"v9_alert_precision_240m_ge10_pct":round(sum((z["future_max_gain"]["240m"] or 0)>=10 for z in a)/len(a)*100,2) if a else 0,"v9_alert_precision_240m_ge20_pct":round(sum((z["future_max_gain"]["240m"] or 0)>=20 for z in a)/len(a)*100,2) if a else 0,"score_band_precision":bands,"v9_alert_episodes":a[:150],"episodes":episodes[:150]}

def main():
    ap=argparse.ArgumentParser();ap.add_argument("--symbols",default="NOMUSDT,NILUSDT,STXUSDT,RAYUSDT,SEIUSDT,SUIUSDT,INJUSDT,AVAXUSDT,PUMPUSDT");ap.add_argument("--start",default="2026-09-20T00:00:00Z");ap.add_argument("--end",default="2026-09-25T00:00:00Z");ap.add_argument("--train-start",default="2026-09-13T00:00:00Z");ap.add_argument("--output",default="data/backtest_results_v9.json");a=ap.parse_args()
    ...
    payload={"generated_at":datetime.now(timezone.utc).isoformat(),"engine":"Pump Replay / Backtest v9","data_source":BASE,"method":"Binance 1m Spot klines with BTC-relative strength, dual EARLY/CONFIRMED paths, persistence, follow-through quality, avoid filter and smoothed historical risk modifier","v9_design":{"v4_weight":0.72,"persistence_weight":0.10,"follow_through_weight":0.13,"historical_risk_modifier":True,"historical_hard_gate":False,"smoothed_prior_hit_rate_pct":10,"early_v4_min_score":60,"early_min_follow_through":65,"early_min_persistence":2,"confirmed_v4_min_score":72,"confirmed_min_persistence":3,"confirmed_min_follow_through":75,"avoid_filter":True,"targets":["10% in 60m","10% in 240m","20% in 240m"]},"calibration":cal,"results":out}
    os.makedirs(os.path.dirname(a.output) or ".",exist_ok=True);json.dump(payload,open(a.output,"w"),indent=2);json.dump(cal,open("data/v9_calibration.json","w"),indent=2);print(json.dumps(payload,indent=2))
